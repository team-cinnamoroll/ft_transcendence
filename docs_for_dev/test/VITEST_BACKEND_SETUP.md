...
このドキュメントでは、`apps/backend` に導入している Vitest の仕様と使い方を説明します。
...
- テスト配置: `apps/backend/test/**/*.test.ts`
- 実装配置: `apps/backend/src/**/*.ts`
...
`apps/backend/package.json` の scripts:
...
1. `apps/backend/test/` に `*.test.ts` を作成
...
- `apps/backend/tsconfig.json` の `compilerOptions.types` を確認
